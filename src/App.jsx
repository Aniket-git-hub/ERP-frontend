...
} from "./api/endpoints/employee/designation"
import {
  getEmployeeOptions,
  getEmployees,
} from "./api/endpoints/employee/employees"
import { getClients } from "./api/endpoints/work/clients"
import {
  getAggregateInvoiceData,
  getInvoices,
} from "./api/endpoints/work/invoices"
...
function App() {
  ...
  return (
    <Routes>
      ...
      <Route
        path="/login"
        element={!isAuthenticated ? <LoginPage /> : <Navigate replace to="/" />}
      />
      <Route
        path="/register"
        element={
          !isAuthenticated ? <RegisterPage /> : <Navigate replace to="/" />
        }
      />
      <Route
        path="/forgot-password"
        element={
          !isAuthenticated ? <ForgotPassword /> : <Navigate replace to="/" />
        }
      />
      <Route path="*" element={<Error404Page />} />
    </Routes>
