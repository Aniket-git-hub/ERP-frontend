import { AbsoluteCenter, Box, Center, Divider, Flex, Heading, List, ListItem, Text } from "@chakra-ui/react"
                        ...
                        <>
                            <Box position='relative' padding='5'>
                                ...
                                <AbsoluteCenter bg='white' px='4'>
                                    <Heading fontWeight={'regular'} color={'gray.500'} size={'sm'}>{setting.name}</Heading>
                                </AbsoluteCenter>
                            </Box>

                            {
                                setting.items && setting.items.map((item, i) => (
                                    <>
                                        <ListItem
                                            color={"gray.700"}
                                            fontWeight={searchParams.get('setting') === item.title ? 'semibold' : ''}
                                            _hover={{ bg: 'purple.200', cursor: "pointer" }}
                                            bg={searchParams.get('setting') === item.title ? 'purple.200' : 'white'}
                                            p={3}
                                            px={5}
                                            my={2}
                                            rounded={"lg"}
                    ...
                    currentSettingPage ? currentSettingPage : <>
                        <Center h={"100%"}>
                            <Box textAlign={"center"}>
                                <Heading fontSize={'5rem'} fontWeight={"bolder"} color={'gray.400'}>Settings</Heading>
                                <Text fontWeight={'regular'} color={"gray.700"}>Choose a settings</Text>
                            </Box>
                        </Center>
                    </>
                }
