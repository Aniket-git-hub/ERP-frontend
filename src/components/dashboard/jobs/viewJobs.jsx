...
function ViewJobs() {
  ...
  const flattenedJobs = (item) => {
    ...
    return item.map((job) => {
      ...
      if (
        job.operation_costs &&
        Array.isArray(job.operation_costs) &&
        job.operation_costs.length > 0
      ) {
        flattenedJob["rate"] = job.operation_costs.reduce(
            ...
            <ModalBody>
              <form onSubmit={handleSubmit}>
                <Heading size={"md"} my={3}>
                  Edit Drawing # {selectedItem.id}
                </Heading>
                <Alert
                  status="error"
                  variant="left-accent"
                  hidden={!selectedItem.InvoiceId}
                >
                  <AlertIcon />
                  Can't Update Billed Jobs - invoiceId #{selectedItem.InvoiceId}
                </Alert>
                <FormControl
                  isInvalid={errors.drawingNumber}
                  isRequired
                  mb=".8rem"
                  isDisabled={selectedItem.InvoiceId !== null}
                >
                ...
                <HStack>
                  ...
                  >
                    ...
                    />
                    <FormErrorMessage>{errors.materialId}</FormErrorMessage>
                  </FormControl>
                </HStack>
                ...
                <Center w={"full"}>
                  <VStack w={"full"}>
                    <Button
                      ...
                      loadingText="Saving..."
                      isDisabled={
                        isSubmitting || selectedItem.InvoiceId !== null
                      }
                    >
                      Save Job
                    </Button>
