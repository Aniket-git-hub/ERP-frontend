...
import { getJobs } from "../../../api/endpoints/work/jobs"
...
function CreateInvoice() {
  ...
  const handleSelectAll = () => {
    ...
    if (allSelected) {
      ...
      const allItemIds = invoiceJobs.items
    ...
    <Container>
      <form onSubmit={handleSubmit}>
        ...
        <ButtonGroup variant={"outline"} isAttached w={"100%"} mt={2} mb={3}>
          <Button
            w={"100%"}
            colorScheme={taxed ? "purple" : "gray"}
            variant={taxed ? "solid" : "outline"}
            onClick={() => setTaxed(true)}
          >
            Tax
          </Button>
          <Button
        ...
        <HStack>
          <FormControl isInvalid={errors.clientId} isRequired mb=".8rem">
            ...
            <Select
              ...
              value={
                clientOptions.filter((c) => c.value === values.clientId)[0]
              }
            />
          ...
          <ModalBody>
            <TableContainer>
              <Table variant={"simple"} size={"sm"}>
                  ...
                  <Tr>
                    <Th whiteSpace="normal" p={3}>
                      <Checkbox
                        isDisabled={
                          invoiceJobs &&
                          invoiceJobs.items.every((item) => item.rate === 0)
                        }
                        isChecked={
                          invoiceJobs &&
                          selectedItems.length === invoiceJobs.items.length
                        }
                    ...
                    <Th whiteSpace="normal" isNumeric p={3}>
                      Qty
                    </Th>
                    <Th whiteSpace="normal" isNumeric p={3}>
                      Rate
                ...
                <Tbody>
                  {invoiceJobs && invoiceJobs.items.length > 0 ? (
                    invoiceJobs.items.map((job, i) => (
                      <Tr key={i}>
                        <Td whiteSpace="normal" p={3} textAlign={"center"}>
                          <Checkbox
                            isChecked={selectedItems.includes(job.id)}
                            onChange={() =>
                              handleCheckboxChange(job.id, job.rate)
                            }
                        ...
                        <Td whiteSpace="normal" isNumeric p={3}>
                          {job.qty}
                        </Td>
                        <Td whiteSpace="normal" isNumeric p={3}>
                          {job?.operation_costs.reduce(
                            (acc, curr) => acc + parseInt(curr.cost),
                            0,
                          )}
                        </Td>
                        <Td whiteSpace="normal" isNumeric p={3}>
