...
function InvoiceById() {
    ...
    return (
        <Box p={5}>
            <Heading size={"md"} mb={3} fontWeight={"semibold"}>Invoice -  {invoiceId}</Heading>
            <Flex w={"100%"}>
                {
                    invoice &&
                    <VStack flex={1} boxShadow={"lg"} p={5} rounded={"lg"}>
                        ...
                        <HStack w={"100%"}>
                            <Text fontWeight={"600"}>{invoice.invoiceType == "taxed" ? "Amount:" : "Total Amount:"} </Text>
                            <Text>{invoice?.totalAmountBeforeTax} </Text>
                        </HStack>

                        <HStack w={"100%"} hidden={invoice.invoiceType == "taxed" ? false : true}>
